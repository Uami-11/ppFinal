...
public:
    ...
    Slash(Vector2 playerPos, Vector2 dir) {
        ...
        // offset slash in front of player
    ...
    void Update(float dt) {
        animTimer += dt;
        if (animTimer > 0.1f) { // 0.1 sec per frame
            animTimer = 0.0f;
            frame++;
            if (frame >= 3) {
                finished = true; // done animating
            }
        }
    }
    ...
    void Draw() {
        ...
        // Draw centered at position, rotated toward direction
        DrawTexturePro(
            tex,
...
public:
    ...
    void Load() {
        ...
        for (int i = 0; i < 4; i++) {
            idleAnim[i] = LoadTexture(("assets/Player/knight_f_idle_anim_f" + to_string(i) + ".png").c_str());
            runAnim[i] = LoadTexture(("assets/Player/knight_f_run_anim_f" + to_string(i) + ".png").c_str()); 
        }
    ...
    void Update() {
        ...
        if (IsKeyPressed(KEY_Z) || IsKeyPressed(KEY_J)){
            ...
            extern vector<Slash> slashes;
            slashes.push_bac    k(Slash(pos, dir));
        }
...
void GameStartup() {
    ...
    camera.offset = {160, 90}; // centers the player
    camera.zoom = 1.0f;
...
void GameUpdate() {
    ...
    // Update slashes
    for (s : slashes) s.Update(dt);

    // Remove finished ones
    slashes.erase(remove_if(slashes.begin(), slashes.end(),
    [](Slash& s){ return s.finished; }), slashes.end());
}
